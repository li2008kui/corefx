...
static void DumpDict(CFStringRef formatString, ...)
{
    va_list args;

    va_start(args, formatString);
    CFStringRef fmt = CFStringCreateWithFormatAndArguments(nullptr, nullptr, CFSTR("%@"), args);
    va_end(args);
    CFDataRef bleh = CFStringCreateExternalRepresentation(nullptr, fmt, CFStringGetSystemEncoding(), '?');

    if (bleh != nullptr)
    {
        printf("%.*s\n", static_cast<int>(CFDataGetLength(bleh)), CFDataGetBytePtr(bleh));
        CFRelease(bleh);
    }
...
AppleCryptoNative_X509ChainEvaluate(SecTrustRef chain, double epochDeltaSeconds, bool allowNetwork, int32_t* pOSStatus)
{
    ...
    *pOSStatus = SecTrustEvaluate(chain, &trustResult);
    ...
    printf("SecTrustGetTrustResult returned %d and emitted %d\n", *pOSStatus, trustResult);
...
extern "C" CFArrayRef AppleCryptoNative_X509ChainGetTrustResults(SecTrustRef chain)
{
    ...
    CFArrayRef details = reinterpret_cast<CFArrayRef>(const_cast<void*>(detailsPtr));

    CFRetain(details);
    CFRelease(detailsAndStuff);

    printf("Just the details:\n");
    DumpDict(nullptr, details);
...
static void MergeStatusCodes(CFTypeRef key, CFTypeRef value, void* context)
{
    ...
    if (*pStatus > INT_MAX)
    {
        printf("Error determined already\n");
        return;
    }
