...
namespace System.Net.Security
{
    ...
    internal class SecureChannel
    {
        ...
        private X509Certificate2 EnsurePrivateKey(X509Certificate certificate)
        {
            ...
            try
            {
                ...
                if (store != null)
                {
                    ...
                    {
                        if (Logging.On) Logging.PrintInfo(Logging.Web, this, SR.Format(SR.net_log_found_cert_in_store, (_serverMode ? "LocalMachine" : "CurrentUser")));
                ...
                if (store != null)
                {
                    ...
                    if (collectionEx.Count > 0 && collectionEx[0].HasPrivateKey)
                    {
                        if (Logging.On) Logging.PrintInfo(Logging.Web, this, SR.Format(SR.net_log_found_cert_in_store, (_serverMode ? "CurrentUser" : "LocalMachine")));
                        return collectionEx[0];
                    }
                }
            }
        ...
        private bool AcquireClientCredentials(ref byte[] thumbPrint)
        {
            ...
            else if (_clientCertificates != null && _clientCertificates.Count > 0)
            {
                ...
                for (int i = 0; i < _clientCertificates.Count; ++i)
                {
                    ...
                    if (Logging.On) Logging.PrintInfo(Logging.Web, this, SR.Format(SR.net_log_selected_cert, _clientCertificates[i].ToString(true)));
        ...
        {
#if TRAVE
            GlobalLog.Enter("SecureChannel#" + Logging.HashString(this) + "::GenerateToken, m_RefreshCredentialNeeded = " + _refreshCredentialNeeded);
#endif
        ...
        internal Interop.SecurityStatus Encrypt(byte[] buffer, int offset, int size, ref byte[] output, out int resultSize)
        {
            ...
            try
            {
                ...
                Buffer.BlockCopy(buffer, offset, e_writeBuffer, _headerSize, size);
            }
            catch (Exception e)
            {
                if (!ExceptionCheck.IsFatal(e))
            ...
            securityBuffer[0] = new SecurityBuffer(e_writeBuffer, 0, _headerSize, SecurityBufferType.Header);
            securityBuffer[1] = new SecurityBuffer(e_writeBuffer, _headerSize, size, SecurityBufferType.Data);
            securityBuffer[2] = new SecurityBuffer(e_writeBuffer, _headerSize + size, _trailerSize, SecurityBufferType.Trailer);
